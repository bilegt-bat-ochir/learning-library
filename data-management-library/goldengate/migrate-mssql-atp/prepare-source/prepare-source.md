...
2. Add the below line and modify it accordingly:
	```
	<copy>
	[mssqlserver_war]            
...
2. You also have to have a schema in the source database that will be used for Oracle GoldenGate objects. The below is only a recommendation.
...
1. Oracle GoldenGate needs to create necessary objects in the source SQL Server for extract process. Microservices deployments already has a default GGSCHEMA oggadmin parameter. It means GoldenGate will try to create database objects in oggadmin schema. If you will not change this, many operations will fail. Therefore, you must create **oggadmin** schema, or must change the default value. Go to **Parameter Files** tab in the administration server configuration window. 
...
1. You need to enable supplemental logging at SQL server, it is called adding trandata. Switch to **Database** tab, and connect to your SQL server first.
